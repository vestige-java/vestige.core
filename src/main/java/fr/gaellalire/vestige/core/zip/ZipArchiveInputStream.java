 ...
 * </p>
 * <p>
 * The {@link ZipFile} class is preferred when reading from files as {@link ZipArchiveInputStream} is limited by not being able to read the central directory header before
...
public class ZipArchiveInputStream extends ArchiveInputStream implements InputStreamStatistics {
    ...
    /*
     * local file header signature WORD version needed to extract SHORT general purpose bit flag SHORT compression method SHORT last mod file time SHORT last mod file date SHORT
     * crc-32 WORD compressed size WORD uncompressed size WORD file name length SHORT extra field length SHORT
     */

    private static final int CFH_LEN = 46;
    /*
     * central file header signature WORD version made by SHORT version needed to extract SHORT general purpose bit flag SHORT compression method SHORT last mod file time SHORT
     * last mod file date SHORT crc-32 WORD compressed size WORD uncompressed size WORD file name length SHORT extra field length SHORT file comment length SHORT disk number start
     * SHORT internal file attributes SHORT external file attributes WORD relative offset of local header WORD
     */
     ...
     * @param useUnicodeExtraFields whether to use InfoZIP Unicode Extra Fields (if present) to set the file names.
     * @param allowStoredEntriesWithDataDescriptor whether the stream will try to read STORED entries that use a data descriptor
     * @param skipSplitSig Whether the stream will try to skip the zip split signature(08074B50) at the beginning. You will need to set this to true if you want to read a split
     *        archive.
     * @since 1.20
     */
    public ZipArchiveInputStream(final InputStream inputStream, final String encoding, final boolean useUnicodeExtraFields, final boolean allowStoredEntriesWithDataDescriptor,
    ...
    public static boolean matches(final byte[] signature, final int length) {
        ...
        return checksig(signature, ZipFile.LFH_SIG) // normal file
                || checksig(signature, ZipFile.EOCD_SIG) // empty zip
     ...
     * <p>
     * If the compressed size of this entry is included in the entry header, then any outstanding bytes are simply skipped from the underlying stream without uncompressing them.
     * This allows an entry to be safely closed even if the compression method is unsupported.
     * <p>
     * In case we don't know the compressed size of this entry or have already buffered too much data from the underlying stream to support uncompression, then the uncompression
     * process is completed and the end position of the stream is adjusted based on the result of that process.
    ...
    private void closeEntry() throws IOException {
        ...
        if (currentEntryHasOutstandingBytes()) {
            ...
            final long inB = current.entry.getMethod() == DEFLATED ? getBytesInflated() : current.bytesRead;
     ...
     * </p>
     * <p>
     * The stream knows how many bytes it has read, but not how many the Inflater actually consumed - it should be between the total number of bytes read for the entry and the
     * total number minus the last read operation. Here we just try to make the value close enough to the bytes we've read by assuming the number of bytes consumed must be smaller
     * than (or equal to) the number of bytes read but not smaller by more than 2^32.
     * </p>
     */
     ...
     * @return true if allowStoredEntriesWithDataDescriptor is true, the entry doesn't require any data descriptor or the method is DEFLATED or ENHANCED_DEFLATED.
     */
    private boolean supportsDataDescriptorFor(final ZipArchiveEntry entry) {
        return !entry.getGeneralPurposeBit().usesDataDescriptor()

                || (allowStoredEntriesWithDataDescriptor && entry.getMethod() == ZipEntry.STORED) || entry.getMethod() == ZipEntry.DEFLATED
    ...
    private boolean supportsCompressedSizeFor(final ZipArchiveEntry entry) {
        return entry.getCompressedSize() != ArchiveEntry.SIZE_UNKNOWN || entry.getMethod() == ZipEntry.DEFLATED || entry.getMethod() == ZipMethod.ENHANCED_DEFLATED.getCode()
    ...
    private static final String USE_ZIPFILE_INSTEAD_OF_STREAM_DISCLAIMER = " while reading a stored entry using data descriptor. Either the archive is broken"
            + " or it can not be read using ZipArchiveInputStream and you must use ZipFile." + " A common cause for this is a ZIP archive containing a ZIP archive."
    ...
    private void readStoredEntry() throws IOException {
        ...
        while (!done) {
            final int r = in.read(buf.array(), off, BUFFER_SIZE - off);
            if (r <= 0) {
    ...
    private boolean bufferContainsSignature(final ByteArrayOutputStream bos, final int offset, final int lastRead, final int expectedDDLen) throws IOException {
        ...
        for (int i = 0; !done && i < offset + lastRead - 4; i++) {
            if (buf.array()[i] == LFH[0] && buf.array()[i + 1] == LFH[1]) {
                int expectDDPos = i;
                if (i >= expectedDDLen && (buf.array()[i + 2] == LFH[2] && buf.array()[i + 3] == LFH[3]) || (buf.array()[i] == CFH[2] && buf.array()[i + 3] == CFH[3])) {
                    // found a LFH or CFH:
    ...
    /**
     * If the last read bytes could hold a data descriptor and an incomplete signature then save the last bytes to the front of the buffer and cache everything in front of the
     * potential data descriptor into the given ByteArrayOutputStream.
     * <p>
    ...
    private void findEocdRecord() throws IOException {
        ...
        while (skipReadCall || (currentByte = readOneByte()) > -1) {
            ...
            currentByte = readOneByte();
            if (currentByte == -1 || currentByte == ZipFile.EOCD_SIG[3]) {
                break;
            }
            skipReadCall = isFirstByteOfEocdSig(currentByte);
        }
    ...
    /**
     * Skips bytes by reading from the underlying stream rather than the (potentially inflating) archive stream - which {@link #skip} would do. Also updates bytes-read counter.
     */
    ...
    /**
     * Reads bytes by reading from the underlying stream rather than the (potentially inflating) archive stream - which {@link #read} would do. Also updates bytes-read counter.
     */
     ...
     * <p>
     * Unfortunately the APK signing block does not start with some kind of signature, it rather ends with one. It starts with a length, so what we do is parse the suspect length,
     * skip ahead far enough, look for the signature and if we've found it, return true.
     * </p>
