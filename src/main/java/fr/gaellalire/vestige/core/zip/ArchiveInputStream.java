...
/**
 * Archive input streams <b>MUST</b> override the {@link #read(byte[], int, int)} - or {@link #read()} - method so that reading from the stream generates EOF for the end of data in
 * each entry as well as at the end of the file proper.
 * <p>
...
public abstract class ArchiveInputStream extends InputStream {
    ...
    /*
     * Note that subclasses also implement specific get() methods which return the appropriate class without need for a cast. See SVN revision r743259
     * @return
     * @throws IOException
     */
    ...
    /**
     * Reads a byte of data. This method will block until enough input is available. Simply calls the {@link #read(byte[], int, int)} method. MUST be overridden if the
     * {@link #read(byte[], int, int)} method is not overridden; may be overridden otherwise.
