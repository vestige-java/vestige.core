 ...
 * </ul>
 * <p>
 * Any extra data that cannot be parsed by the rules above will be consumed as "unparseable" extra data and treated differently by the methods of this class. Versions prior to
 * Apache Commons Compress 1.1 would have thrown an exception if any attempt was made to read or write extra data not conforming to the recommendation.
 * </p>
 * @NotThreadSafe
 */
public class ZipArchiveEntry extends java.util.zip.ZipEntry implements ArchiveEntry, EntryStreamOffsets {
    ...
    public enum NameSource {
        /**
         * The name has been read from the archive using the encoding of the archive specified when creating the {@link ZipArchiveInputStream} or {@link ZipFile} (defaults to the
    ...
    public enum CommentSource {
        /**
         * The comment has been read from the archive using the encoding of the archive specified when creating the {@link ZipArchiveInputStream} or {@link ZipFile} (defaults to
    ...
    /**
     * The {@link java.util.zip.ZipEntry#setSize} method in the base class throws an IllegalArgumentException if the size is bigger than 2GB for Java versions &lt; 7 and even in
     * Java 7+ if the implementation in java.util.zip doesn't support Zip64 itself (it is an optional feature).
     * <p>
     ...
     * <p>
     * The name will be adjusted to end with a forward slash "/" if the file is a directory. If the file is not a directory a potential trailing forward slash will be stripped from
     * the entry name.
     * </p>
     ...
     * <p>
     * <b>Note</b>: The set of extra fields may be incomplete when {@link ZipArchiveInputStream} has been used as some extra fields use the central directory to store additional
     * information.
     * </p>
    ...
    /**
     * Unfortunately {@link java.util.zip.ZipOutputStream java.util.zip.ZipOutputStream} seems to access the extra data directly, so overriding getExtra doesn't help - we need to
     ...
     * <p>
     * <b>Note</b>: {@link ZipArchiveInputStream} may create entries that return {@link #SIZE_UNKNOWN SIZE_UNKNOWN} as long as the entry hasn't been read completely.
     * </p>
    ...
    /**
     * If there are no extra fields, use the given fields as new extra data - otherwise merge the fields assuming the existing fields and the new fields stem from different
     * locations inside the archive.
     ...
     * <p>
     * Changes to the implementation of {@link java.util.zip.ZipEntry#getTime} leak through and the returned value may depend on your local time zone as well as your version of
     * Java.
     * </p>
     */
    @Override
    ...
    public boolean equals(final Object obj) {
        ...
        return getTime() == other.getTime() && myComment.equals(otherComment) && getInternalAttributes() == other.getInternalAttributes() && getPlatform() == other.getPlatform()
                && getExternalAttributes() == other.getExternalAttributes() && getMethod() == other.getMethod() && getSize() == other.getSize() && getCrc() == other.getCrc()
                && getCompressedSize() == other.getCompressedSize() && Arrays.equals(getCentralDirectoryExtra(), other.getCentralDirectoryExtra())
                && Arrays.equals(getLocalFileDataExtra(), other.getLocalFileDataExtra()) && localHeaderOffset == other.localHeaderOffset && dataOffset == other.dataOffset
     ...
     * <li>What shall happen if an extra field is generally supported by Commons Compress but its content cannot be parsed correctly? This may for example happen if the archive is
     * corrupt, it triggers a bug in Commons Compress or the extra field uses a version not (yet) supported by Commons Compress.</li>
     * </ul>
     * @since 1.19
     */
    public enum ExtraFieldParsingMode implements ExtraFieldParsingBehavior {
        /**
         * Try to parse as many extra fields as possible and wrap unknown extra fields as well as supported extra fields that cannot be parsed in {@link UnrecognizedExtraField}.
         * <p>
