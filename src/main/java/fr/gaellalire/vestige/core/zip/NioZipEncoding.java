...
class NioZipEncoding implements ZipEncoding, CharsetAccessor {
    ...
    private CharsetEncoder newEncoder() {
        if (useReplacement) {
            return charset.newEncoder().onMalformedInput(CodingErrorAction.REPLACE).onUnmappableCharacter(CodingErrorAction.REPLACE).replaceWith(REPLACEMENT_BYTES);
        } else {
    ...
    private CharsetDecoder newDecoder() {
        if (!useReplacement) {
            ...
            return charset.newDecoder().onMalformedInput(CodingErrorAction.REPLACE).onUnmappableCharacter(CodingErrorAction.REPLACE).replaceWith(REPLACEMENT_STRING);
        }
    }
     ...
     * <p>
     * The estimate assumes that one character consumes uses the maximum length encoding, whilst the rest use an average size encoding. This accounts for any BOM for UTF-16, at the
     * expense of a couple of extra bytes for UTF-8 encoded ASCII.
     * </p>
