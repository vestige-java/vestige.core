...
import static fr.gaellalire.vestige.core.zip.ZipConstants.DWORD;
import static fr.gaellalire.vestige.core.zip.ZipConstants.SHORT;
import static fr.gaellalire.vestige.core.zip.ZipConstants.WORD;
import static fr.gaellalire.vestige.core.zip.ZipConstants.ZIP64_MAGIC;
import static fr.gaellalire.vestige.core.zip.ZipConstants.ZIP64_MAGIC_SHORT;
 ...
 * found in self extracting archives. Furthermore it returns instances of <code>org.apache.commons.compress.archivers.zip.ZipArchiveEntry</code> instead of
 * <code>java.util.zip.ZipEntry</code>.
 * </p>
 * <p>
 * It doesn't extend <code>java.util.zip.ZipFile</code> as it would have to reimplement all methods anyway. Like <code>java.util.ZipFile</code>, it uses SeekableByteChannel under
 * the covers and supports compressed and uncompressed entries. As of Apache Commons Compress 1.3 it also transparently supports Zip64 extensions and thus individual entries and
 * archives larger than 4 GB or with more than 65536 entries.
 * </p>
 * <p>
...
public class ZipFile implements Closeable {
     ...
     * <a href="http://java.sun.com/j2se/1.5.0/docs/guide/intl/encoding.doc.html">http://java.sun.com/j2se/1.5.0/docs/guide/intl/encoding.doc.html</a>. Defaults to UTF-8.
     * </p>
     */
     ...
     * </p>
     * <p>
     * By default the central directory record and all local file headers of the archive will be read immediately which may take a considerable amount of time when the archive is
     * big. The {@code ignoreLocalFileHeader} parameter can be set to {@code true} which restricts parsing to the central directory. Unfortunately the local file header may contain
     * information not present inside of the central directory which will not be available when the argument is set to {@code true}. This includes the content of the Unicode extra
     * field, so setting {@code
     * ignoreLocalFileHeader} to {@code true} means {@code useUnicodeExtraFields} will be ignored effectively. Also {@link #getRawInputStream} is always going to return
     * {@code null} if {@code ignoreLocalFileHeader} is {@code
     * true}.
     * </p>
     * @param channel the archive.
     * @param archiveName name of the archive, used for error messages only.
     ...
     * @since 1.19
     */
    public ZipFile(final SeekableByteChannel channel, final String archiveName, final String encoding, final boolean useUnicodeExtraFields, final boolean ignoreLocalFileHeader)
            throws IOException {
     ...
     * <p>
     * The ZipArchiveEntrys will know all data that can be obtained from the central directory alone, but not the data that requires the local file header or additional data to be
     * read.
     * </p>
     ...
     * @param noUTF8Flag map used to collect entries that don't have their UTF-8 flag set and whose name will be set by data read from the local file header later. The current
     *        entry may be added to this map.
     */
    ...
    /**
     * If the entry holds a Zip64 extended information extra field, read sizes from there if the entry's sizes are set to 0xFFFFFFFFF, do the same for the offset of the local file
     * header.
     * <p>
     * Ensures the Zip64 extra either knows both compressed and uncompressed size or neither of both as the internal logic in ExtraFieldUtils forces the field to create local
     * header data even if they are never used - and here a field with only one size would be invalid.
     * </p>
     */
    ...
    /**
     * Offset of the field that holds the location of the first central directory entry inside the "End of central directory record" relative to the start of the "End of central
     * directory record".
     */
    ...
    /**
     * Offset of the field that holds the location of the "Zip64 end of central directory record" inside the "Zip64 end of central directory locator" relative to the start of the
    ...
    /**
     * Offset of the field that holds the location of the first central directory entry inside the "Zip64 end of central directory record" relative to the start of the "Zip64 end
    ...
    /**
     * Searches for either the &quot;Zip64 end of central directory locator&quot; or the &quot;End of central dir record&quot;, parses it and positions the stream at the first
    ...
    private void positionAtCentralDirectory() throws IOException {
        ...
        if (searchedForZip64EOCD) {
            ...
            wordBbuf.rewind();
            IOUtils.readFully(archive, wordBbuf);
            found = Arrays.equals(ZIP64_EOCD_LOC_SIG, wordBuf);
        }
        if (!found) {
            // not a ZIP64 archive
            if (searchedForZip64EOCD) {
    ...
    /**
     * Parses the &quot;Zip64 end of central directory locator&quot;, finds the &quot;Zip64 end of central directory record&quot; using the parsed information, parses that and
     * positions the stream at the first central directory record. Expects stream to be positioned right behind the &quot;Zip64 end of central directory locator&quot;'s signature.
     */
    ...
    /**
     * Parses the &quot;End of central dir record&quot; and positions the stream at the first central directory record. Expects stream to be positioned at the beginning of the
    ...
    private void positionAtEndOfCentralDirectoryRecord() throws IOException {
        final boolean found = tryToLocateSignature(MIN_EOCD_SIZE, MAX_EOCD_SIZE, EOCD_SIG);
        if (!found) {
    ...
    /**
     * Lock-free implementation of BoundedInputStream. The implementation uses positioned reads on the underlying archive file channel and therefore performs significantly faster
